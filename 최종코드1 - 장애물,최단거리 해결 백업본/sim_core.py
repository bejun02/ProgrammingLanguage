...
def simulate(cfg: FactoryConfig):
    ...
    cache_path = os.path.join(script_dir, "path_cache.pkl")
    
    try:
        with open(cache_path, "rb") as f:
            global_variable.path_cache = pickle.load(f)
        log(f"Loaded {len(global_variable.path_cache)} paths from cache.")
    except FileNotFoundError:
        log(f"Warning: path_cache.pkl not found at {cache_path}. Using on-the-fly pathfinding.")
        global_variable.path_cache = {}
        
    global_variable.path_finder = PathFinder(cfg.machine_positions)
...
def move_to_next_stage_from_output(m: Machine):
    ...
    if nxt is None: 
        ...
        def go_pickup():
            ...
            record_amr_run(amr, job, depart_at, arrive_pick, future_start, m.output_port, loaded=False, path=path)
        ...
        def pickup_end():
            if m.output_buf is job:
                ...
                record_amr_run(amr, job, depart_pick, arrive_drop, m.output_port, drop_xy, loaded=True, path=path)
...
def try_dispatch_from_warehouse_to_A():
    ...
    def go_pickup():
        ...
        path = global_variable.path_cache.get((amr.xy, pick_xy), None)
        record_amr_run(amr, job, depart_at, arrive_pick, future_start, pick_xy, loaded=False, path=path)
        amr.xy = pick_xy
    ...
    def pickup_end():
        ...
        path = global_variable.path_cache.get((pick_xy, drop_xy), None)
        record_amr_run(amr, job, depart_pick, arrive_drop, pick_xy, drop_xy, loaded=True, path=path)
...
def pull_from_prev_to(m_next: Machine, policy: str = "eta"):
    ...
    def go_pickup():
        ...
        path = global_variable.path_cache.get((amr.xy, src_m.output_port), None)
        record_amr_run(amr, job, depart_at, arrive_pick, future_start, src_m.output_port, loaded=False, path=path)
    ...
    def pickup_end():
        if src_m.output_buf is job:
            ...
            path = global_variable.path_cache.get((src_m.output_port, drop_xy), None)
            record_amr_run(amr, job, depart_pick, arrive_drop, src_m.output_port, drop_xy, loaded=True, path=path)

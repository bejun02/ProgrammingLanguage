...
from heuristic import (
    select_best_machine, select_best_amr, 
    update_avail_time, update_cycle_count, reset_heuristic_state,
    heuristic_params, heuristic_state, set_distance_func
)

# 휴리스틱 사용 여부 (True: 휴리스틱, False: 라운드로빈)
USE_HEURISTIC = True
...
def simulate(cfg: FactoryConfig):
    ...
    if USE_HEURISTIC:
        ...
        # pathfinding 모듈과 연동: 다익스트라 경로 거리 함수 주입
        set_distance_func(dist)  # sim_core의 dist() 함수 사용
        log(f"[Heuristic] 활성화됨 (α={heuristic_params.alpha}, β={heuristic_params.beta}, γ={heuristic_params.gamma}, δ={heuristic_params.delta})")
        log(f"[Heuristic] pathfinding 연동: {USE_PATHFINDING}")
...
def on_finish_processing(m: Machine):
    ...
    # ===== 휴리스틱 상태 업데이트 =====
    if USE_HEURISTIC:
        # 처리시간 누적 업데이트 (비율 보정용) - 시간 기준
        process_time = process_time_for(m.stage, job, m)
        update_cycle_count(m.stage, job.cycle_idx, process_time)
        # 설비 avail_time 업데이트
        update_avail_time(m, global_variable.now)
...
def reserve_amr(pick_xy: Tuple[float, float], drop_xy: Tuple[float, float],
                ...
                job_id: Optional[str] = None, target_stage: Optional[str] = None):
    """
    AMR 예약 - 휴리스틱 기반 최적 AMR 선택 (pathfinding 적용)
    ...
    Args:
        ...
        target_stage (Optional[str]): 목적지 공정 스테이지 (AMR 휴리스틱용)
    ...
    AMR 선택 기준:
        USE_HEURISTIC=True: select_best_amr() 휴리스틱 사용 (병목 공정 우선)
        USE_HEURISTIC=False: ETA 최소화 (기존 방식)
    
    타임라인 계산 (pathfinding 적용):
        depart_at = max(request_time, amr.free_time)  # AMR 가용 시점
        arrive_pick = depart_at + dist(현재위치, 픽업위치) / 속도  # dist()는 pathfinding 사용
    ...
    # ===== AMR 선택 (ETA 최소화 + pathfinding 적용) =====
    # Note: AMR 휴리스틱(select_best_amr)은 현재 성능 저하를 유발하므로
    # 기존 ETA 최소화 방식 유지. 단, pathfinding으로 정확한 거리 계산.
    # 
    # 향후 AMR 휴리스틱 적용 시: USE_HEURISTIC and target_stage 조건 사용
    # amr = select_best_amr(amrs=global_variable.AMRS, pick_xy=pick_xy,
    #                       drop_xy=drop_xy, target_stage=target_stage, now=request_time)
    ...
    for a in global_variable.AMRS:
        ...
        # pathfinding 적용: dist() 함수는 USE_PATHFINDING에 따라 다익스트라 사용
        ...
        depart_drop = arrive_drop + unload_sec
        cands.append((depart_drop, a))
    cands.sort(key=lambda x: x[0])
    amr = cands[0][1]
    
    # ===== 선택된 AMR로 타임라인 계산 (pathfinding 적용) =====
    depart_at = max(request_time, amr.free_time)
    future_start = amr.planned_xy if (amr.planned_xy is not None and amr.free_time > request_time) else amr.xy
    
    # pathfinding 적용: dist() 함수는 USE_PATHFINDING=True일 때 다익스트라 경로 거리 사용
    t_pick = dist(future_start, pick_xy) / max(amr.speed, 1e-9)
    t_drop = dist(pick_xy, drop_xy) / max(amr.speed, 1e-9)
...
def try_start_processing(m: Machine):
    ...
    if m.processing_job is None and m.input_buf:
        ...
        # 휴리스틱 상태 업데이트: 가공 종료 예정 시간 갱신
        if USE_HEURISTIC:
            heuristic_state.avail_time[m.name] = e
...
def move_to_next_stage_from_output(m: Machine):
    ...
    if slot_ok:
        # ===== 휴리스틱 vs 라운드로빈 선택 =====
        if USE_HEURISTIC:
            # 휴리스틱 기반 설비 선택 (Score 최소화)
            drop_m = select_best_machine(
                job=job,
                current_machine=m,
                target_stage=nxt,
                candidate_machines=slot_ok,
                now=global_variable.now
            )
            if drop_m is None:
                drop_m = slot_ok[0]  # fallback
            log(f"{nxt}: 입력 슬롯 여유 {drop_m.name} 선택 (휴리스틱, free {len(slot_ok)}대)")
        else:
            # 라운드로빈으로 설비 선택 (부하 분산)
    ...
    res = reserve_amr(m.output_port, drop_xy, request_time=global_variable.now,
                      load_sec=load_sec, unload_sec=unload_sec, job_id=job.job_id,
                      target_stage=nxt)  # AMR 휴리스틱용 (병목 공정 우선)

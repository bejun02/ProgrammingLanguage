...
def process_time_for(stage: str, job: Job, m: Optional[Machine] = None) -> float:
    ...
    │ ProdB    │  5 │ 40 │ 25 │  2 │ 15 │ 10 │ 20 │ 10 │ 10 │ 15 │
    └──────────┴────┴────┴────┴────┴────┴────┴────┴────┴────┴────┘
...
# 로깅 및 기록 함수
# ================================================================================
# 성능 최적화: VERBOSE=False로 설정하면 로그 출력 생략 (실행 속도 10배 이상 향상)
VERBOSE = False  # False로 변경하면 로그 출력 안 함
...
def log(msg: str):
    ...
    Note:
        VERBOSE=False로 설정하면 출력 생략 (성능 최적화)
    """
    if VERBOSE:

...
def next_stage_for(job: Job, current_stage: str) -> Optional[str]:
    """현재 스테이지에서 Job이 다음에 갈 스테이지를 반환
    
    중요: pending_stage가 이미 설정되어 있으면 그대로 반환 (중복 계산 방지)
    이 체크가 없으면 next_stage_for()가 여러 번 호출될 때 cycle_idx가 중복 증가함
    """
...
def process_time_for(stage: str, job: Job, m: Optional[Machine] = None) -> float:
    ...
    try:
        ...
        print(f"ERROR: process_time_for 계산 중 오류 - {e}")
    
    # 기본값 반환 (15분 = 900초) - 예외 발생 시에도 동작하도록
    return 15 * 60 
...
# 로그 출력 제어 플래그 (False로 설정하면 10배 이상 빨라짐)
VERBOSE = True

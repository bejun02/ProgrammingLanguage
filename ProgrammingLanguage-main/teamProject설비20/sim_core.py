...
def simulate(cfg: FactoryConfig):
    ...
    # 금지구역 맵 초기화 (pathfinding)
    if USE_PATHFINDING and cfg.machine_positions:
...
def dist(a: Tuple[float, float], b: Tuple[float, float]) -> float:
    '''거리 계산 (USE_PATHFINDING=True면 다익스트라, False면 유클리드)'''
